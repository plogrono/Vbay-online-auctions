USE vbay
GO
--Walk through to get familiar with all tables
...
--Join vb_items and vb_users tables to determine which emails belong to sellers 
select s.*, i.*
from vb_items as i 
    join vb_users as s
...
--Join vb_items and vb_users twice to identify sellers and buyers of items sold
select *
from vb_items as i 
    join vb_users as s 
        on s.user_id = i.item_seller_user_id
    join vb_users as b 
        on b.user_id = i.item_buyer_user_id;



--Join user table to show buyers and sellers, and find the profit/loss margin for each item sold
...
from vb_items as i 
    join vb_users as s 
        on s.user_id = i.item_seller_user_id
    join vb_users as b 
        on b.user_id = i.item_seller_user_id
where item_sold >= 1;
...
for example 13244, so they can be notified of their new contact in that region.*/
select * 
from vb_users 
where user_zip_code like '13%';

/*2. Find all users from state of NY. Print their names, emails, city, state and zip code.  
Sort by city, then user's last/first name*/
...
/*3. High-priced items: Return the ID, name, type, and reserve of items that have not been sold and 
have a reserve of 250 or higher.  Sort by city, then by user's last/first name.*/
...
/*4. Reserve item categories, do not include 'all other' and add column 250 or more "high-priced item";
50 or less "low-priced item"; everything else "average-priced item"*/
select item_id, item_name, item_type, item_reserve,
    case when item_reserve >= 250 then 'high-priced item'
    when item_reserve <=50 then 'low-priced item'
            else 'average-priced item'
            end as reserve_item_category
from vb_items
where NOT item_type = 'All Other';

/* 5. Bidder list. Write a query that displays the valid user bids (bid status of 'ok') for a given item_id
This would commonly be displayed on the website for the chosen item.  You select the item ID to display and show the bid ID, 
bid user's name, bid user email, bid date, and bid amount.  Put the most recent bids at the top  */
...
/*6. Bad bidder list. Write a query to help the security audit team find fraudulent activity.  
For any bid that does not have a status of ‘ok’, include the date of the bid, name, email, and ID of the bidder and the name and ID of the item bid upon. 
Also include the amount of the bid and bid status.  
Sort the output by the user name (last, then first) and then by bid date for users with multiple bad bids.*/
...
/*7. Items with no bids. Produce a report of items that do not contain a bid. Include the item ID, item name, item type,
seller’s name, and item reserve.*/
...
/*8. Seller ratings.  Produce a list of seller ratings. Include the name of the user who gave the rating, the name of the
user the rating was for, the rating value, and rating comment. Include ratings of only sellers*/
select concat(g.user_firstname, ' ', g.user_lastname) as rating_giver, concat(t.user_firstname, ' ', t.user_lastname) as person_rated, 
r.rating_value, r.rating_comment, r.rating_astype
...
/*9. For items that were sold, generate a report that includes the locations (city and state) of the buyer
and seller. Include item ID, item name, item type item sold amount, name of seller, seller’s
...
/*10. Users with no activity. Find the names and emails of any users who have never posted an item for
bid or have never bought the item or have never placed a bid.*/
...
-----walk through to more complex queries-------
...
/*Vbay would like to classify their users based on the number of valid bids they have placed.
Low Activity - 0 or 1 bid
...
--Produce Activity report from previous query by creating table using "WITH" statement to name query
with user_bids as (
    select s.user_email, s.user_firstname, s.user_lastname, count(*) as bid_counts,
        case when count(*) between 0 and 1 then 'Low'
            when count(*) between 2 and 4 then 'Moderate'
...
/*Using previous query as a table, count how many users for each user_bid_activity category sorted by user_count*/
with user_bids as (
    select s.user_email, s.user_firstname, s.user_lastname, count(*) as bid_counts,
        case when count(*) between 0 and 1 then 'Low'
            when count(*) between 2 and 4 then 'Moderate'
...
/*1. How many item types are there? For each item type, provide the count of items,in that type, the min, average and max item reserve prices for that type. 
...
/*2. Perform an analysis of each item in the “Antiques” and “Collectables” item types. For each item, display the name, item type, and item reserve. 
...
/*3. Write a query to include the names, counts (number of ratings), and average seller ratings (as a decimal) of users. For reference, User Carrie Dababbi 
...
/*4. Create a list of “Collectable” item types with more than one bid. Include the name of the item and the number of bids, 
making sure the item with the most bids appear first.*/
...
/*5. Generate a valid bidding history for any given item of your choice. Display the item ID, item name, a number representing the order the bid was placed, 
...
/*6. Rewrite your query in the previous question to include the names of the next and previous bidders, like this example, 
...
/*7. Find the names and emails of the users who give out the worst ratings (lower than the overall average rating) to either buyers or sellers 
...
/*8. Produce a report of the KPI (key performance indicator) user bids per item. Show the user’s name and email, total number of valid bids, 
...
/*9. Among items not sold, show highest bidder name and the highest bid for each item. Make sure to include only valid bids.*/
...
/*10. Write a query with output similar to Question 3, but also includes the overall average seller 
